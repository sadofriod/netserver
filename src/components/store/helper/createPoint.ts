...
export const createPoint: Common.ReducerHelper<{ path: string; type: "input" | "output" }> = (payload, state) => {
	const { currentNode, nodes } = state;
	...
	const newNodeHeight = (sourceKeyCount + 1) * nodeBoxOffsetHeight;

	const pointStyle: Common.PointStyle = {
		x: type === "input" ? style.x + pointRadin : style.x + style.width - pointRadin,
		y: style.y + style.height / 2,
		color: type === "input" ? "#fff" : "#2eb2af",
	};
